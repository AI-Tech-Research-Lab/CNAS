...
from utils import AverageMeter, accuracy, Logger, save_checkpoint, adjust_learning_rate, adjust_meta_learning_rate, calc_target_probs, get_scores_string, get_num_string, get_free_port
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    ip = '127.0.0.1'
    port = args.port
    hosts = 1                                           # number of gpu servers
    rank = 0                                            # rank of current server
    ...
    # init the model for the experiments (e.g., MSDNet)
    model = getattr(models, args.arch)(args)
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        if i % args.meta_interval == 0:
            ...
            if args.meta_net_input_type == 'loss':
                ...
                input_of_meta = pseudo_losses # INPUT OF THE WPN
            else:
                for j in range(args.num_exits):
                    pseudo_loss_vector = F.cross_entropy(pseudo_outputs[j], target_p1, reduction='none')
