...
def ofa_net(n_classes = 1000, net_id = 'ofa_mbv3_d234_e346_k357_w1.0', pretrained=True):
    ...
    if net_id == 'ofa_proxyless_d234_e346_k357_w1.3':
        net = OFAProxylessNASNets(
            n_classes, dropout_rate=0, width_mult_list=1.3, ks_list=[3, 5, 7], expand_ratio_list=[3, 4, 6], depth_list=[2, 3, 4],
        )
        ...
        net = OFAMobileNetV3(
            n_classes, dropout_rate=0, width_mult_list=1.2, ks_list=[3, 5, 7], expand_ratio_list=[3, 4, 6], depth_list=[2, 3, 4],
        )
    elif net_id == 'ofa_resnet50_d=0+1+2_e=0.2+0.25+0.35_w=0.65+0.8+1.0':
        net = OFAResNets(
          n_classes, dropout_rate=0, depth_list=[0, 1, 2], expand_ratio_list=[0.2, 0.25, 0.35], width_mult_list=[0.65, 0.8, 1.0]
        )
    ...
    if pretrained:
        ...
        init = torch.load(
            download_url(url_base + net_id, model_dir='./ofa_nets'),
