...
if os.path.exists('ckpt_fp.pth'):
    ...
    # Check model parameters and buffers
    ...
    # Check optimizer state tensors
    for state_key, state_value in optimizer.state.items():
        for sub_key, sub_value in state_value.items():
            if isinstance(sub_value, torch.Tensor):
                print(f"Optimizer state {sub_key} for parameter {state_key} is on device: {sub_value.device}")
else:
    print('start training')
    train(train_loader,val_loader,1,quantized_model,device,criterion,optimizer,100,'ckpt_fp.pth')
...
if os.path.exists('ckpt_qt.pth'):
    ...
    print('start training')
    train(train_loader,val_loader,1,quantized_model,device,criterion,optimizer,100,'ckpt_qt.pth')
