...
def plot_histogram(data, bins=36, path=''):
    import matplotlib.pyplot as plt
    import numpy as np
    ...
    # Plot the histogram
    sns.histplot(data, bins=bins, color='darkblue', edgecolor='black', kde=True, line_kws={'linewidth': 4}, ax=ax)

    # Hide y-axis values
    ax.tick_params(axis='y', which='both', left=False, right=False, labelleft=False)

    # Add X-axis label
    ax.set_xlabel('Val acc', fontsize=FONT_SIZE)

    # Adjust layout
    plt.tight_layout()

    # Save the plot to a file
    plt.savefig(os.path.join(path, 'histogram.pdf'), dpi=300, bbox_inches='tight')
...
bench=NASBench201(dataset='cifar10')
plot_histogram(bench.archive['val-acc']['cifar10-valid'])