...
def fog(x, severity=1):
    ...
    img_size=x.shape[0]
    max_val = x.max()
    x += c[0] * plasma_fractal(wibbledecay=c[1])[:img_size, :img_size][..., np.newaxis]
...
def frost(x, severity=1):
    ...
    #frost = cv2.resize(frost, (0, 0), fx=0.2, fy=0.2) #rescale (fx,fy) the frost image before cropping
    # randomly crop and convert to rgb
    x_start, y_start = np.random.randint(0, frost.shape[0] - img_size), np.random.randint(0, frost.shape[1] - img_size)
    frost = frost[x_start:x_start + img_size, y_start:y_start + img_size][..., [2, 1, 0]]
...
def pixelate(x, severity=1):
    ...
    # Convert the NumPy array 'x' to a Pillow Image
    img = PILImage.fromarray(x)
    
    # Get the size of the image
    img_size = img.size
    
    # Resize the image to achieve pixelation
    new_size = (int(img_size[0] * c), int(img_size[1] * c))
    img = img.resize(new_size, PILImage.BOX)
    
    # Resize back to the original size to get the pixelated effect
    img = img.resize(img_size, PILImage.BOX)
    
    # Convert the pixelated image back to a NumPy array
    x = np.array(img)
...
"""
How a corrupt function looks like:
...
def glass_blur(x, severity=1):
    ...
    x = np.uint8(gaussian(np.array(x) / 255., sigma=c[0], channel_axis=-1) * 255) #multichannel deprecated
...
def apply_distortion(x, severity, dist_name):
    if dist_name == 'gaussian_noise':
        return gaussian_noise(x, severity)
    elif dist_name == 'shot_noise':
        return shot_noise(x, severity)
    elif dist_name == 'impulse_noise':
        return impulse_noise(x, severity)
    elif dist_name == 'contrast':
        return contrast(x, severity)
    elif dist_name == 'elastic_transform':
        return elastic_transform(x, severity)
    elif dist_name == 'speckle_noise':
        return speckle_noise(x, severity)
    elif dist_name == 'defocus_blur':
        return defocus_blur(x, severity)
    elif dist_name == 'glass_blur':
        return glass_blur(x, severity)
    elif dist_name == 'motion_blur':
        return motion_blur(x, severity)
    elif dist_name == 'zoom_blur':
        return zoom_blur(x, severity)
    elif dist_name == 'snow':
        return snow(x, severity)
    elif dist_name == 'fog':
        return fog(x, severity)
    elif dist_name == 'brightness':
        return brightness(x, severity)
    elif dist_name == 'pixelate':
        return pixelate(x, severity)
    elif dist_name == 'jpeg_compression':
        return jpeg_compression(x, severity)
    elif dist_name == 'gaussian_blur':
        return gaussian_blur(x, severity)
    elif dist_name == 'spatter':
        return spatter(x, severity)
    elif dist_name == 'saturate':
        return saturate(x, severity)
    elif dist_name == 'frost':
        return frost(x, severity)
    else:
        raise ValueError(f"Unknown distortion type: {dist_name}")

# Implement the specific distortion functions (e.g., gaussian_noise, shot_noise, etc.)
# You'll need to define these functions or import them from a module where they are implemented.

class ApplyDistortion(object):
    def __init__(self, dist_name, severity):
        self.dist_name=dist_name
        self.severity=severity

    def __call__(self, x):
        x = apply_distortion(x, self.severity, self.dist_name)
        #x=np.transpose(x, (2, 0, 1)) #this is performed by the ToTensor() transform
        x=x/255
