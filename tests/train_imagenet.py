...
parser.add_argument('--model-config', default='', type=str,
...
parser.add_argument('--teacher', type=str, default=None,
                    help='which model')
parser.add_argument('--reset-classifier', action='store_true', default=False,
                    help='whether to reset the classifier')
...
def main():
    ...
    if args.teacher:
        ...
        teacher, _ = supernet.sample({'ks': [7]*20, 'e': [6]*20, 'd': [4]*5})
    ...
    data_config = resolve_data_config(vars(args), model=dummy_model, verbose=args.local_rank == 0)
    if args.img_size is not None:
        data_config['input_size'] = (3, args.img_size, args.img_size)  # override the input image resolution
    ...
    if args.num_gpu > 1:
        ...
        if args.teacher: teacher = nn.DataParallel(teacher, device_ids=list(range(args.num_gpu))).cuda()
    else:
        model.cuda()
    ...
    try:
        for epoch in range(start_epoch, num_epochs):
            ...
            train_metrics = train_epoch(
                ...
                use_amp=use_amp, model_ema=model_ema, teacher=teacher)
...
def train_epoch(
        ...
        lr_scheduler=None, saver=None, output_dir='', use_amp=False, model_ema=None, teacher=None):
    ...
    if args.kd_ratio > 0: teacher.train()
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        if args.kd_ratio > 0:
            with torch.no_grad():
                soft_logits = teacher(input).detach()
        ...
        if args.kd_ratio > 0:
            # using the supernet at full scale to supervise the training of the subnets
            # this is taken from https://github.com/mit-han-lab/once-for-all/blob/
            # 4decacf9d85dbc948a902c6dc34ea032d711e0a9/ofa/imagenet_codebase/run_manager/run_manager.py#L426
            kd_loss = cross_entropy_loss_with_soft_target(output, soft_label)
            loss = args.kd_ratio * kd_loss + loss_fn(output, target)
