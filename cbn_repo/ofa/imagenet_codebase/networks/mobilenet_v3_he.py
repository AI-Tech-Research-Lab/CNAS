...
class MobileNetV3HE(MyNetwork):
    ...
    def build_from_config(config):
        ...
        net = MobileNetV3HE(first_conv, blocks, final_expand_layer, feature_mix_layer, classifier)
        if 'bn' in config:
    ...
    def build_net_via_cfg(cfg, input_channel, last_channel, n_classes, dropout_rate):
        ...
        first_conv = ConvLayer(
            3, input_channel, kernel_size=3, stride=2, use_bn=True, act_func='square', ops_order='weight_bn_act'
        )
        # build mobile blocks
        ...
        final_expand_layer = ConvLayer(
            feature_dim, feature_dim * 6, kernel_size=1, use_bn=True, act_func='square', ops_order='weight_bn_act',
        )
        feature_dim = feature_dim * 6
        # feature mix layer
        feature_mix_layer = ConvLayer(
            feature_dim, last_channel, kernel_size=1, bias=False, use_bn=False, act_func='square',
        )
        # classifier
...
class MobileNetV3Large(MobileNetV3HE):
    ...
    def __init__(self, n_classes=1000, width_mult=1, bn_param=(0.1, 1e-3), dropout_rate=0.2,
        ...
        cfg = {
            #    k,     exp,    c,      se,         nl,         s,      e,
            '0': [
                [3,     16,     16,     False,      'square',     1,      1],
            ],
            '1': [
                [3,     64,     24,     False,      'square',     2,      None],  # 4
                [3,     72,     24,     False,      'square',     1,      None],  # 3
            ],
            '2': [
                [5,     72,     40,     True,       'square',     2,      None],  # 3
                [5,     120,    40,     True,       'square',     1,      None],  # 3
                [5,     120,    40,     True,       'square',     1,      None],  # 3
            ],
            '3': [
                [3,     240,    80,     False,      'square',  2,      None],  # 6
                [3,     200,    80,     False,      'square',  1,      None],  # 2.5
                [3,     184,    80,     False,      'square',  1,      None],  # 2.3
                [3,     184,    80,     False,      'square',  1,      None],  # 2.3
            ],
            '4': [
                [3,     480,    112,    True,       'square',  1,      None],  # 6
                [3,     672,    112,    True,       'square',  1,      None],  # 6
            ],
            '5': [
                [5,     672,    160,    True,       'square',  2,      None],  # 6
                [5,     960,    160,    True,       'square',  1,      None],  # 6
                [5,     960,    160,    True,       'square',  1,      None],  # 6
            ]
        }
        ...
        super(MobileNetV3LargeHE, self).__init__(first_conv, blocks, final_expand_layer, feature_mix_layer, classifier)
        # set bn param
