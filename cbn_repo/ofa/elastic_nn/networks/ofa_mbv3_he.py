...
from ofa.imagenet_codebase.networks.mobilenet_v3_he import MobileNetV3HE, MobileInvertedResidualBlock
from ofa.imagenet_codebase.utils import make_divisible, int2list


class OFAMobileNetV3HE(MobileNetV3HE):
    ...
    def __init__(self, n_classes=1000, bn_param=(0.1, 1e-5), dropout_rate=0.1, base_stage_width=None,
        ...
        act_stages = ['square', 'square', 'square', 'square', 'square', 'square']
        se_stages = [False, False, True, False, True, True]
        if depth_list is None:
            n_block_list = [1, 2, 3, 4, 2, 3]
            self.depth_list = [4, 4]
            print('Use MobileNetV3HE Depth Setting')
        else:
        ...
        if len(set(input_channel)) == 1:
            first_conv = ConvLayer(3, max(input_channel), kernel_size=3, stride=2, act_func='square')
        ...
        if len(final_expand_width) == 1:
            final_expand_layer = ConvLayer(max(feature_dim), max(final_expand_width), kernel_size=1, act_func='square')
            feature_mix_layer = ConvLayer(
                max(final_expand_width), max(last_channel), kernel_size=1, bias=False, use_bn=False, act_func='square',
            )
        else:
            final_expand_layer = DynamicConvLayer(
                in_channel_list=feature_dim, out_channel_list=final_expand_width, kernel_size=1, act_func='square'
            )
            feature_mix_layer = DynamicConvLayer(
                in_channel_list=final_expand_width, out_channel_list=last_channel, kernel_size=1,
                use_bn=False, act_func='square',
            )
        ...
        super(OFAMobileNetV3HE, self).__init__(first_conv, blocks, final_expand_layer, feature_mix_layer, classifier)
    ...
    def config(self):
        return {
            'name': OFAMobileNetV3HE.__name__,
    ...
    def get_active_subnet(self, preserve_weight=True):
        ...
        _subnet = MobileNetV3HE(first_conv, blocks, final_expand_layer, feature_mix_layer, classifier)
